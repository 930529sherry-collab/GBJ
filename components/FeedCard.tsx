...
import { MapPinIcon, TrophyIcon, UserGroupIcon } from './icons/NavIcons';
import { HeartIcon, ChatBubbleIcon, PaperAirplaneIcon } from './icons/ActionIcons';

interface FeedCardProps {
    item: FeedItem;
    isLink?: boolean;
    onToggleLike: (itemId: number) => void;
    onAddComment: (itemId: number, commentText: string, storeName?: string) => void;
    ...
    return (
        <div className="bg-brand-secondary p-4 rounded-xl border-2 border-brand-accent/20 shadow-lg shadow-brand-accent/10 flex flex-col gap-3">
            <div className="flex justify-between items-center">
                {headerLink}
                <span className="text-xs text-brand-muted">{item.timestamp}</span>
            </div>
            ...
            {item.imageUrl && (
                <div className="mt-2 pl-12">
                    <img src={item.imageUrl} alt="Feed post" className="w-full h-auto max-h-80 rounded-lg object-cover border-2 border-brand-accent/10" />
                </div>
            )}
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="relative flex-grow">
                                    ...
                                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                        ...
                                        <button type="submit" className="p-1 text-brand-accent disabled:text-brand-muted" disabled={!newComment.trim() && !selectedStoreId}>
